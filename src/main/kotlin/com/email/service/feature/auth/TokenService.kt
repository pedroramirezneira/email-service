...
class TokenService(
    ...
    fun issueAccessToken(subject: String, roles: Set<Role>): Result<String, Unit> {
        return try {
            val now = Instant.now()
            ...
            Result.Success(jwtEncoder.encode(JwtEncoderParameters.from(header, claims)).tokenValue)
        } catch (e: Exception) {
            Result.Failure(Unit)
        }
    }
